...
def _verify_tiingo_token():
    """
    CRITICAL: Verify that Tiingo API token is present and consistent.
    Prevents silent configuration errors that could cause cascading API failures.
    
    Raises:
        RuntimeError: If token is missing, invalid, or mismatched
    """
    import os
    
    # Get token from environment
    env_token = os.getenv('TIINGO_API_TOKEN')
    
    if not env_token:
        error_msg = (
            "\n🔴 CRITICAL TOKEN ERROR:\n"
            "   Environment variable TIINGO_API_TOKEN is not set.\n"
            "   Expected: /etc/opticore.env or systemd EnvironmentFile\n"
            "   Action: Set TIINGO_API_TOKEN before starting service"
        )
        logger.critical(error_msg)
        raise RuntimeError(error_msg)
    
    # Get token from config (which also reads from environment)
    config_token = Config.TIINGO_API_TOKEN
    
    # Verify consistency
    if env_token != config_token:
        error_msg = (
            f"\n🔴 TOKEN MISMATCH ERROR:\n"
            f"   Environment: {_mask_token(env_token)}\n"
            f"   Config loaded: {_mask_token(config_token)}\n"
            f"   This indicates a configuration inconsistency that MUST be fixed."
        )
        logger.critical(error_msg)
        raise RuntimeError(error_msg)
    
    # Log masked token at startup
    logger.info(f"🔑 Tiingo API Token Loaded: {_mask_token(env_token)}")
    logger.info(f"✅ Token consistency verified - environment and config match")
...
class MLPipelineScheduler:
    ...
    def __init__(self, enable_telegram: bool = True):
        ...
        # CRITICAL: Verify Tiingo API token is present and consistent
        _verify_tiingo_token()
...
{"─" * 40}"""
    ...
    def register_jobs(self):
        ...
        # Job 1b: Fetch 1h data every hour (at :10 UTC - race-safe offset before event_monitor_1h at :15 UTC)
        self.scheduler.add_job(
            self.fetch_tiingo_data,
        ...
        # Job 1c: Fetch 4h data every 4 hours (at :00 UTC, synced with data availability)
        self.scheduler.add_job(
            self.fetch_tiingo_data,
        ...
        # Job 9a: Session snapshot for ASIAN session (22:00 UTC)
        self.scheduler.add_job(
            self.session_snapshot_job,
            trigger=CronTrigger(hour=22, minute=0, timezone='UTC'),
            args=["ASIAN"],
            id='session_snapshot_asian',
            name='Session Snapshot - Asian',
            max_instances=1,
            coalesce=True,
            misfire_grace_time=300
        )
        logger.info(f"   ✅ Job registered: session_snapshot_asian (22:00 UTC)")
        
        # Job 9b: Session snapshot for LONDON session (08:00 UTC)
        self.scheduler.add_job(
            self.session_snapshot_job,
            trigger=CronTrigger(hour=8, minute=0, timezone='UTC'),
            args=["LONDON"],
            id='session_snapshot_london',
            name='Session Snapshot - London',
            max_instances=1,
            coalesce=True,
            misfire_grace_time=300
        )
        logger.info(f"   ✅ Job registered: session_snapshot_london (08:00 UTC)")
        
        # Job 9c: Session snapshot for NEW_YORK session (13:00 UTC)
        self.scheduler.add_job(
            self.session_snapshot_job,
            trigger=CronTrigger(hour=13, minute=0, timezone='UTC'),
            args=["NEW_YORK"],
            id='session_snapshot_newyork',
            name='Session Snapshot - New York',
            max_instances=1,
            coalesce=True,
            misfire_grace_time=300
        )
        logger.info(f"   ✅ Job registered: session_snapshot_newyork (13:00 UTC)")
