...
class PureStrategyEngine:
    ...
    def broadcast_trade_signal(self, signal_data: Dict[str, Any], signal_id: int = None) -> None:
        ...
        try:
            ...
            pip_size = getattr(Config, "PIP_SIZE_MAP", {}).get(ticker, 0.0001)
            sl_pips  = abs(entry - sl) / pip_size if pip_size > 0 else 0
            tp_pips  = abs(tp - entry) / pip_size if pip_size > 0 else 0
            tf       = getattr(Config, "TIMEFRAME_CONTEXT", {}).get(interval, {})

            alert_msg = (
                f"{'='*40}\n"
                f"SIGNAL BROADCAST - SILENT ANALYST\n"
                f"{'='*40}\n"
                f"ID         : SA-{trade_id[:8]}\n"
                f"INSTRUMENT : {ticker}\n"
                f"DIRECTION  : {'BUY' if direction == 1 else 'SELL'}\n"
                f"{'-'*40}\n"
                f"TIMEFRAME  : {interval.upper()} - {tf.get('label', '')}\n"
                f"HOLD EST.  : {tf.get('hold', '')}\n"
                f"BEHAVIOUR  : {tf.get('behaviour', '')}\n"
                f"{'-'*40}\n"
                f"ENTRY      : {entry:.4f}\n"
                f"STOP LOSS  : {sl:.4f}  ({sl_pips:.0f} pips)\n"
                f"TAKE PROFIT: {tp:.4f}  ({tp_pips:.0f} pips)\n"
                f"R:R        : {rr:.2f} : 1\n"
                f"{'-'*40}\n"
                f"TRIGGER    : Breakout Confirmation\n"
                f"CONFIDENCE : 100%\n"
                f"ENGINE     : {'V2 State Machine' if 'v2' in strategy_name.lower() else 'V1 Core'}\n"
                f"STATUS     : ACTIVE\n"
                f"{'='*40}"
            )
            ...
            if success:
                ...
                try:
                    ...
                    strategy_version = 'v2' if 'v2' in model_version.lower() else 'v1'
                    ...
                    forensics = {
                        ...
                        'ema100_4h': feature_snapshot.get('ema_100_4h'),
        ...
        *,
        ...
        if strategy_name == 'strategy_core_v1':
            ...
            try:
                ...
                v3_score, v3_signals = v3_engine.evaluate(df_ohlcv)
                pip_size = self._get_pip_size(ticker)
                
                # Log V3 evaluation
                logging.info(
                    f"[V3_LOGGING] {ticker}-{interval} | "
                    f"Score: {v3_score}/8 | "
                    f"Signals: {v3_signals} | "
                    f"PipSize: {pip_size}"
                )
                
                # Log to database forensics
                if v3_signals and len(v3_signals) > 0:
                    for sig in v3_signals:
                        try:
                            self.db.log_signal_forensic(
                                ticker=ticker,
                                interval=interval,
                                score=v3_score,
                                compression=sig.get('compression', 0),
                                adx_slope_ok=sig.get('adx_slope_ok', 0),
                                hidden_div=sig.get('hidden_div', 0),
                                confluence_score=v3_score,
                            )
                        except Exception as e:
                            logging.debug(f"[V3_LOGGING] Failed to log forensics: {e}")
                
                # Phase 1: Logging-only (return None to prevent actual signal generation)
                return None
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
        # If no event triggered this evaluation, do NOT emit a signal
        # (No time-based scheduled signals allowed)
        if event is None:
            return None
    ...
    def handle_event(
        ...
        *,
        ...
        if STRATEGY_V2_AVAILABLE:
            ...
            try:
                from core.strategy_core_v2 import SignalEngine as SignalEngineV2, get_timeframe_config
                ...
                v2_config = get_timeframe_config(event.interval)
                v2_engine = SignalEngineV2(event.ticker, event.interval, v2_config)
                
                # Get OHLCV data
                df_ohlcv = self.get_latest_ohlcv(event.ticker, event.interval)
                ...
                if df_ohlcv is not None and len(df_ohlcv) >= 120:
                    ...
                    # If signal generated, process it through trade pipeline
                    if core_signal != 0 and core_signal is not None:
                        signal_v2 = self.execute_trade_pipeline(
                            ticker=event.ticker,
                            interval=event.interval,
                            strategy_name='strategy_core_v2',
                            direction=int(core_signal),
