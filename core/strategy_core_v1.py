...
============================================================
Original 5-condition AND gate — VOLUME REMOVED (April 2026)

FIXES APPLIED:
    1. Volume filter completely removed (unreliable on forex)
...
class V1SignalEngine:
    ...
    def evaluate(self, df_ohlcv: pd.DataFrame) -> Signal:
        ...
        if pd.isna(ema_21) or pd.isna(rsi_14):
            logger.debug(f"[V1] {self.ticker}-{self.interval}: Indicator NaN — ema={ema_21}, rsi={rsi_14}")
            return Signal.NEUTRAL
        ...
        if (close > ema_21 and 
            ...
            is_bull_engulf and 
            daily_bull):
            logger.info(f"[V1_BUY] {self.ticker}-{self.interval}: close={close:.5f}, ema={ema_21:.5f}, rsi={rsi_14:.1f}, daily_bull={daily_bull}")
            return Signal.BUY
        ...
        if (close < ema_21 and 
            ...
            is_bear_engulf and 
            daily_bear):
            logger.info(f"[V1_SELL] {self.ticker}-{self.interval}: close={close:.5f}, ema={ema_21:.5f}, rsi={rsi_14:.1f}, daily_bear={daily_bear}")
            return Signal.SELL
