...
class Config:
    ...
    OBSERVATION_CHANNEL_ID = "-1003717292831"  # Observation channel
    ...
    CRYPTO_FETCH_INTERVALS: list = ["5m", "15m", "30m", "1h", "2h", "4h", "D"]
    ...
    TELEGRAM_NOTIFICATIONS_ENABLED = True   # Signal broadcast enabled
    WATCHLIST_TELEGRAM_ENABLED = False      # Watchlist private bot broadcast -- set True to re-enable
    ...
    # ==========================================
    # Shadow mode: logs VOLUME_BREAKOUT events to pressure_alert_log
    # but does NOT send push notifications until empirically validated.
    # Set to False only after threshold audit confirms signal quality.
    VOLUME_BREAKOUT_SHADOW_MODE   = True

    # Minimum volume_ratio to qualify as a breakout spike
    # volume_ratio = current_bar_volume / 20-bar rolling average
    # 2.0 = current volume is 2x average. Start conservative.
    VOLUME_BREAKOUT_THRESHOLD     = 2.0

    # Maximum compression_score to qualify (must be compressed or resolving)
    VOLUME_BREAKOUT_MAX_COMPRESS  = 0.50

    # Minimum expansion_pressure to qualify (momentum building)
    VOLUME_BREAKOUT_MIN_EXPANSION = 0.30

    # Minimum pressure states that must be present for volume event to matter
    # Volume spike on a NEUTRAL asset is noise. Must already be on radar.
    VOLUME_BREAKOUT_VALID_STATES  = ('RISING', 'HIGH', 'EXTREME')

    # ==========================================
    # ATR STOP LOSS CALIBRATION
    # ==========================================
    # Multiplier applied to ATR when calculating stop distance.
    # Design intent: 1.5 (per Source of Truth v2).
    # Previous deployed value: 0.5 (deployment error -- 3x too tight).
    # Empirically validated: NEAR case confirms 1.5x survives
    # expansion-phase oscillations that 0.5x cannot.
    ATR_MULTIPLIER = 1.5

    # Maximum stop loss as percentage of entry price.
    # Prevents outlier stops on low-priced tokens (e.g. sub-$0.10).
    # At 0.5x multiplier, HUSDT/ESPORTSUSDT produced 32%+ SL distances.
    # At 1.5x this risk is amplified -- cap enforces a hard ceiling.
    # 8.0% = generous for crypto, prevents catastrophic outliers.
    SL_MAX_PCT = 0.08
    ...
    # FLOW-IGNITE — Intraday Capital Flow Detection System
    # OptiCore Labs · Silent Analyst · Added: May 24 2026
    # ═══════════════════════════════════════════════════════════

    # ── FLOW ENGINE MASTER SWITCH ────────────────────────────
    FLOW_ENGINE_ENABLED             = True

    # ── FLOW SCAN INTERVALS ──────────────────────────────────
    # Primary detection interval (15m candle close)
    FLOW_PRIMARY_INTERVAL           = '15m'
    # Session filter interval (directional context only)
    FLOW_SESSION_INTERVAL           = '30m'

    # ── FLOW WATCHLIST ELIGIBILITY THRESHOLDS ────────────────
    # Instruments must pass ALL three to enter the Flow universe
    # Source: empirically derived — Brief 2A, May 24 2026
    FLOW_WICK_THRESHOLD             = 4.0   # avg wick/body ratio <=
    FLOW_MIN_DAILY_ATR_PCT          = 3.0   # estimated daily ATR % >=
    FLOW_MIN_RANGE_30BAR_PCT        = 5.0   # 30-bar price range % >=

    # ── FLOW WATCHLIST EXCLUSION LIST ────────────────────────
    # Instruments confirmed to produce negative edge on 15m
    # body-break logic — empirically derived Brief 3C May 24 2026
    # Full ticker names including USDT suffix
    FLOW_WATCHLIST_EXCLUSIONS       = [
        'ICPUSDT',   # 36.4% pos rate, -0.39% avg 3h
        'TIAUSDT',   # 30.0% pos rate, -0.44% avg 3h
        'INJUSDT',   # 42.1% pos rate, -0.23% avg 3h
        'DOTUSDT',   # 33.3% pos rate, 0.00% avg 3h
    ]

    # ── SIGNAL DETECTION THRESHOLDS ──────────────────────────
    # Volume confirmation — break candle must exceed this multiple
    # Source: empirically calibrated — Brief 3C, May 24 2026
    FLOW_VOLUME_MULTIPLIER          = 3.0

    # Key level lookback window (bars)
    # 48 bars x 15m = 12 hours of session structure
    FLOW_LEVEL_LOOKBACK_BARS        = 48

    # Body commitment minimum (body / ATR14)
    FLOW_BODY_COMMITMENT_MIN        = 0.40

    # EMA period for alignment filter
    FLOW_EMA_PERIOD                 = 21

    # ── CLV SCORING (NOT A HARD GATE) ────────────────────────
    # CLV is a score modifier and badge trigger only
    # It does NOT veto signals — it classifies them
    FLOW_HIGH_CONVICTION_CLV        = 0.85  # triggers HIGH_CONVICTION flag

    # ── STOP LOSS STRUCTURE ───────────────────────────────────
    # STRUCTURAL stop — NOT pure ATR
    # LONG:  min(break_candle_low, key_level) - (ATR14 * buffer)
    # SHORT: max(break_candle_high, key_level) + (ATR14 * buffer)
    # Source: empirically mandated — Brief 3C SL hit rate finding
    FLOW_STOP_ATR_BUFFER            = 0.5   # ATR multiplier for buffer

    # Maximum SL distance as % of entry (safety cap)
    FLOW_SL_MAX_PCT                 = 0.12  # 12% — wider than Pulse/Rift
                                            # because stop is structural

    # ── TAKE PROFIT STRUCTURE ─────────────────────────────────
    # 3R target — empirically justified by fat-tail 24h distribution
    # Source: Brief 3C forward outcome analysis May 24 2026
    FLOW_TP_MULTIPLIER              = 3.0   # TP = entry + (risk x 3.0)

    # ── TELEGRAM ROUTING ──────────────────────────────────────
    # Private bot — receives ALL Flow signals from signal one
    # Community channel remains disabled until manual activation
    # Set this to your private Telegram chat ID before Phase 8
    FLOW_PRIVATE_TELEGRAM_CHAT_ID   = None  # REQUIRED: set before deploy
    FLOW_COMMUNITY_BROADCAST_ENABLED = False  # Activated after 1wk review

    # ── FORENSIC TRACKING ─────────────────────────────────────
    # Required from day one — behavioral analysis depends on this
    FLOW_FORENSICS_ENABLED          = True

    # ── STRATEGY VERSION IDENTIFIER ───────────────────────────
    # Written to ml_signals.strategy_version for all Flow signals
    # Follows existing v1/v3 pattern — Flow is v4
    FLOW_STRATEGY_VERSION           = 'v4'

    # ── MINIMUM DATA REQUIREMENTS ─────────────────────────────
    # Minimum bars required before a symbol is evaluated
    FLOW_MIN_BARS_REQUIRED          = 100

    # ── DEDUPLICATION COOLDOWN ────────────────────────────────
    # Minimum bars between signals on same symbol/direction
    # Prevents back-to-back signals on the same structural event
    FLOW_SIGNAL_COOLDOWN_BARS       = 4     # 4 x 15m = 1 hour cooldown

    # ── MOMENTUM GATE ─────────────────────────────────────────────
    # Symbol must show >= this % move from UTC daily open to be
    # eligible for Flow scan universe on a given day.
    # Matches DailyScanner momentum logic for Pulse and Rift.
    # UTC calendar day open = first 30m bar at/after 00:00 UTC.
    # Source: empirically matched to Pulse/Rift -- May 26 2026
    FLOW_MIN_MOMENTUM_PCT           = 4.0   # abs(change from open) >= 4%

    # ── BREAKOUT DISTANCE GATE ────────────────────
    # Close must clear the key level by >= this % after a body break.
    # Prevents marginal "grazing" breaks from firing signals.
    # Empirical basis: TRUMPUSDT close was only 0.09% above level.
    # 0.3% minimum ensures the close has meaningful commitment
    # beyond the structural level -- not just a tick through.
    FLOW_MIN_BREAK_PCT              = 0.003  # close clears level by >= 0.3%
