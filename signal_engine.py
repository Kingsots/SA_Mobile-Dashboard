...
class PureStrategyEngine:
    ...
    def generate_signal(
        ...
        *,
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # NEW: TRADE CONSTRUCTION PIPELINE
        # Build complete trade signal (entry, SL, TP) from strategy direction
        # ═══════════════════════════════════════════════════════════════════════
        ...
        trade_signal = build_trade_signal(
            ...
            interval=interval,
            df_ohlcv=df_ohlcv,
            direction=core_signal,
            strategy_name=strategy_name,
            confidence=0.95 if strategy_name == 'strategy_core_v2' else 1.0,
        )
        
        if trade_signal is None:
            logging.debug(f"Trade signal construction failed for {ticker}-{interval}")
            return None
        
        # Validate trade setup against risk rules
        if not validate_trade_signal(trade_signal):
            logging.warning(
                f"Trade signal rejected for {ticker}-{interval}: {trade_signal.rejection_reason}"
            )
            return None
        
        # Check if breakout trade has expired
        if is_trade_expired(trade_signal):
            logging.info(
                f"Trade expired: {trade_signal.trade_id} ({ticker}-{interval}). "
        ...
        # Build signal data with complete trade construction
        signal_data: Dict[str, Any] = {
            ...
            # NEW: Complete trade execution prices
            ...
            'entry_type': trade_signal.entry_type,  # Breakout confirmation indicator
            'signal_candle_time': trade_signal.signal_candle_time,  # Unix timestamp of signal candle
            'expiry_timestamp': trade_signal.expiry_timestamp,  # When breakout order expires
            'expiry_candles': trade_signal.expiry_candles,  # Number of candles until expiry
